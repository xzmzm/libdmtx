...
dmtx_decode(const unsigned char *pixels,
    ...
    //int(*callbackFunc)(dmtx_decoded_t *decode_result))
{
    ...
    img = dmtxImageCreate((unsigned char *)pixels, (int)width, (int)height, DmtxPack8bppK);// DmtxPack24bppRGB);
    if (img == NULL) return DMTX_RETURN_NO_MEMORY;

    dmtxImageSetProp(img, DmtxPropRowPadBytes, 0);// bitmapStride % 3);
    ...
    result_count = 0;
    std::vector<dmtx_decoded_t> resultList;
    ...
    while ((region != NULL) && (result_count < max_results)) {
        ...
        std::cout << "dmtxDecodeMatrixRegion" << std::endl;
        ...
        if (msg != NULL) {
            result.data = (char*)CoTaskMemAlloc(msg->outputSize);
            if (result.data != NULL) {
        ...
        std::cout << "resultList.push_back" << std::endl;
    ...
    // Clean-up
    std::cout << "dmtxRegionDestroy" << std::endl;
    dmtxRegionDestroy(&region);
    dmtxDecodeDestroy(&decode);
    dmtxImageDestroy(&img);

    *results = (dmtx_decoded_t*)CoTaskMemAlloc(resultList.size() * sizeof(dmtx_decoded_t));
    std::memcpy(*results, resultList.data(), resultList.size() * sizeof(dmtx_decoded_t));
    ...
    const dmtx_encode_options_t *options)
{
    ...
    *result = NULL;
    ...
    if (options->mosaic)
        err = dmtxEncodeDataMosaic(enc, (int)text_size, (unsigned char *)plain_text);
    else
        err = dmtxEncodeDataMatrix(enc, (int)text_size, (unsigned char *)plain_text);
    if (err != DmtxPass) {
        dmtxEncodeDestroy(&enc);
        return DMTX_RETURN_ENCODE_ERROR;
    }

    res = *result = (dmtx_encoded_t*)malloc(sizeof(dmtx_encoded_t));
    if (res == NULL) {
        dmtxEncodeDestroy(&enc);
